...
def load_asset_ids(excel_file):
    ...
    try:
        # Load the file into pandas (engine='openpyxl' usually needed for xlsx)
        df = pd.read_excel(excel_file, header=None, dtype=str)
        # Assumes IDs are in the first column (index 0)
        asset_ids = (
            df.iloc[:, 0]
            .astype(str)
            .str.replace('\xa0', ' ')
...
def process_redlabel_csv(file, asset_ids):
    ...
    # We need to read it twice: once for header, once for data (per your original script)
    # Since 'file' is a stream, we must reset the pointer
    file.seek(0)
    
    try:
        # Read header (row 2, so skiprows=1)
